...
class PersonAdapter(context: Context, val item: ArrayList<Person>) : BaseAdapter() {
    ...
    override fun getView(position: Int, view: View?, parent: ViewGroup?): View {
        val View: View?
        View = layoutinflator.inflate(R.layout.list_item_layout, parent, false)
        val viewholder: ListviewViewHolder = ListviewViewHolder(View)
        viewholder.txtname?.text = PersonList[position].name
        viewholder.txtaddress?.text = PersonList[position].address
        viewholder.imgView?.setImageResource(PersonList[position].image)
        return View
    }
