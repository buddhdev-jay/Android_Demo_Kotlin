        ...
        intent.getStringExtra("UserId")?.let { userdata ->
            retrofit.getUser(userdata.toInt()).enqueue(object : Callback<SingleUserData>{
                override fun onResponse(call: Call<SingleUserData>, response: Response<SingleUserData>) {
                    response.body().let {
                        Picasso.get().load(it?.data?.avatar).into(single_user_img)
                        txt_singleuser_email.text = it?.data?.email
                        txt_singleuser_fname.text = it?.data?.firstName
                        txt_singleuser_lname.text = it?.data?.lastName
                    }
                }
